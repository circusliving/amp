...
import webPageByPath  from '~/apollo/webpageByPath.js'
...
async function getWebPage({ apolloProvider }, path){
  const page = (await apolloProvider.defaultClient.query({
    query    : webPageByPath,
    variables: { path }
  })).data.webPage
...
async function getItems(app, collNamesArr, tags){
  ...
  const collObjs    = Object.values(await queryColls(apollo, collNames, tags))
  ...
  for (const collection of collObjs)
    ...
    { count += collection.count }
      ...
      const name    = collNames.pop()
      const collMap = mapMollection(collection, name)
